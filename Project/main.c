...
int randomUnformaly();
void reset();
SemaphoreHandle_t MutexSender1;
SemaphoreHandle_t MutexSender2;
SemaphoreHandle_t MutexSender3;
SemaphoreHandle_t MutexResiver;
...
int main(int argc, char* argv[])
{
	// Add your code here.
	    BaseType_t status;
	    MutexSender1 = xSemaphoreCreateBinary();
		MutexSender2 = xSemaphoreCreateBinary();
		MutexSender3 = xSemaphoreCreateBinary();
		MutexResiver = xSemaphoreCreateBinary();
		testQueue = xQueueCreate( 10, sizeof(char) * 20 ); //holds 5 int32
		if( testQueue != NULL ) {
		reset();
...
BaseType_t xStatus;
while(1){
	xSemaphoreTake(MutexSender1,portMAX_DELAY);
	//char *MessageToSend=(char *)malloc(15);
	//snprintf(MessageToSend, "Time is %u",xTaskGetTickCount());
...
BaseType_t xStatus;
while(1){
	xSemaphoreTake(MutexSender2,portMAX_DELAY);
	xTimeNow=xTaskGetTickCount();
...
void SenderTask3()
{
	...
	BaseType_t xStatus;
while(1){
	xSemaphoreTake(MutexSender3,portMAX_DELAY);
	xTimeNow=xTaskGetTickCount();
	//char *MessageToSend=(char *)malloc(15);
	//snprintf(MessageToSend, "Time is %u",xTaskGetTickCount());
...
BaseType_t status;
while(1){
	xSemaphoreTake(MutexResiver,portMAX_DELAY);
